...
#ifdef COAP_DEBUG_LOG_RF_PRINT
		GPRS_DEBUG(DEBUG_INFO, "Coap FullFunc Check Ok");
#endif
	}
	else {
		/* Dictate execute is Fail */
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
	#if NBIOT_PRINT_ERROR_CODE_TYPE
		GPRS_DEBUG(DEBUG_INFO, "Coap FullFunc Check Fail ECde %d", NBStatus);
	#else
		GPRS_DEBUG(DEBUG_INFO, "Coap FullFunc Check Fail");
	#endif
#endif
		return;
	}
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
			GPRS_DEBUG(DEBUG_INFO, "Coap FullFunc Set Ok");
#endif
		}
		else {
			/* Dictate execute is Fail */
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
		#if NBIOT_PRINT_ERROR_CODE_TYPE
			GPRS_DEBUG(DEBUG_INFO, "Coap FullFunc Set Fail ECde %d", NBStatus);
		#else
			GPRS_DEBUG(DEBUG_INFO, "Coap FullFunc Set Fail");
		#endif
#endif
			return;
		}
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
		GPRS_DEBUG(DEBUG_INFO, "Coap MinFunc Check Ok");
#endif
	}
	else {
		/* Dictate execute is Fail */
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
	#if NBIOT_PRINT_ERROR_CODE_TYPE
		GPRS_DEBUG(DEBUG_INFO, "Coap MinFunc Check Fail ECde %d", NBStatus);
	#else
		GPRS_DEBUG(DEBUG_INFO, "Coap MinFunc Check Fail");
	#endif
#endif
		return;
	}
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
			GPRS_DEBUG(DEBUG_INFO, "Coap MinFunc Set Ok");
#endif
		}
		else {
			/* Dictate execute is Fail */
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
		#if NBIOT_PRINT_ERROR_CODE_TYPE
			GPRS_DEBUG(DEBUG_INFO, "Coap MinFunc Set Fail ECde %d", NBStatus);
		#else
			GPRS_DEBUG(DEBUG_INFO, "Coap MinFunc Set Fail");
		#endif
#endif
			return;
		}
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
			GPRS_DEBUG(DEBUG_INFO, "Coap BAND Set %d Ok", COAP_NBIOT_BAND_TYPE);
#endif
		}
		else {
			/* Dictate execute is Fail */
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
		#if NBIOT_PRINT_ERROR_CODE_TYPE
			GPRS_DEBUG(DEBUG_INFO, "Coap BAND Set %d Fail ECde %d", COAP_NBIOT_BAND_TYPE, NBStatus);
		#else
			GPRS_DEBUG(DEBUG_INFO, "Coap BAND Set %d Fail", COAP_NBIOT_BAND_TYPE);
		#endif
#endif
			return;
		}
...
void NET_COAP_NBIOT_Event_RecvDataRANormal(NBIOT_ClientsTypeDef* pClient)
{
	...
	if (pClient->Parameter.condatastate == SendSussess) {
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
		GPRS_DEBUG(DEBUG_INFO, "Coap Send Ok");
#endif
	}
	else {
		...
		if (Stm32_Calculagraph_IsExpiredSec(&pClient->DictateRunCtl.dictateRunTime) == true) {
			...
			pClient->DictateRunCtl.dictateEvent = RECV_DATA_RA_NORMAL;
#ifdef COAP_DEBUG_LOG_RF_PRINT
			GPRS_DEBUG(DEBUG_INFO, "Coap Wait Send Ok");
#endif
		}
	}
}
	...
	if ((pClient->ListenRunCtl.ListenEnterIdle.listenEnable == true) && (pClient->ListenRunCtl.ListenEnterIdle.listenStatus == true)) {
		if (Stm32_Calculagraph_IsExpiredSec(&pClient->ListenRunCtl.ListenEnterIdle.listenRunTime) == true) {
			...
			if (pClient->Parameter.connectedstate == IdleMode) {
				...
				pClient->ListenRunCtl.listenEvent = ENTER_IDLE_MODE;
#ifdef COAP_DEBUG_LOG_RF_PRINT
				GPRS_DEBUG(DEBUG_INFO, "NB Enter IDLE Mode");
#endif
			}
			else {
				...
				#endif
#ifdef COAP_DEBUG_LOG_RF_PRINT
				GPRS_DEBUG(DEBUG_INFO, "NB Not Enter IDLE Mode");
#endif
			}
		}
	}
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
				GPRS_DEBUG(DEBUG_INFO, "Coap Patameter Check Ok");
...
#ifdef COAP_DEBUG_LOG_RF_PRINT
				GPRS_DEBUG(DEBUG_INFO, "Coap Patameter Check Fail");
#endif
				return;
			}
